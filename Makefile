...
LFLAGS=$(WALL) $(LIBJLIB2) -o jhead $(LIBS)

ALL_OBJ=jhead$(OBJ) jhead_a$(OBJ) jhead_h$(OBJ) jhead_i$(OBJ)
ALL_C=jhead.c jhead_a.c jhead_h.c jhead_i.c
ALL_H=jhead.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) jhead.1$(GZ)
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) jhead
	$(CP) jhead $(PRODUCTION)/jhead
	$(CHMOD) 755 $(PRODUCTION)/jhead
	$(CP) jhead.1$(GZ) $(PRODMAN)/jhead.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/jhead.1$(GZ)

uninstall:	all
	-$(RM) $(PRODUCTION)/jhead
	-$(RM) $(PRODMAN)/jhead.1.gz
	-$(RM) $(PRODMAN)/jhead.1
